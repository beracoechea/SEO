...
const PS_MARKER = "###SEO_MONITOR_PS###";

function installerCmdHeader(): string {
  return [
    "@echo off",
    "setlocal EnableExtensions",
    "title Instalar motor SEO",
    ...
    "  exit /b 1",
    ")",
    "set \"SEO_INSTALL_PS=%TEMP%\\seo-monitor-install.ps1\"",
    "echo Preparando instalador...",
    "\"%PS%\" -NoProfile -ExecutionPolicy Bypass -Command \"$c=Get-Content -LiteralPath '%~f0' -Raw -Encoding UTF8; $m='###'+'SEO_MONITOR_PS'+'###'; if ($c.IndexOf($m) -lt 0) { throw 'instalador danado' }; [IO.File]::WriteAllText($env:SEO_INSTALL_PS, $c.Substring($c.IndexOf($m)+$m.Length).TrimStart(), (New-Object Text.UTF8Encoding $false))\"",
    "if errorlevel 1 (",
...
export function buildClientInstaller(input: InstallerInput): string {
  ...
  const lines = [
    ...
    "$TaskName = 'SEO Monitor runtime update'",
    "$StartTaskName = 'SEO Monitor runtime'",
    ...
    "  $req.UserAgent = 'Mozilla/5.0 SEO-Monitor-installer'",
    ...
    "  $busy = @(Get-PortPids)",
    "  if ($busy.Count -gt 0) {",
    "    Write-UpdateLog 'El puerto 8080 esta ocupado y no responde; se recicla el proceso.'",
    "    Stop-RuntimeSafe",
    "  }",
    ...
    "  $p.StartInfo = $psi",
    "  if (-not $p.Start()) { throw 'No se pudo arrancar uvicorn.' }",
    ...
    "    $keepAlive = New-ScheduledTaskTrigger -Once -At ((Get-Date).AddMinutes(1)) -RepetitionInterval (New-TimeSpan -Minutes 2) -RepetitionDuration (New-TimeSpan -Days 3650)",
    ...
    "    $updateSettings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -ExecutionTimeLimit (New-TimeSpan -Hours 2)",
    "    Register-ScheduledTask -TaskName $StartTaskName -Action $startAction -Trigger @($logon, $keepAlive) -Settings $startSettings -User $env:USERNAME -RunLevel Limited -Force | Out-Null",
    "    Register-ScheduledTask -TaskName $TaskName -Action $updateAction -Trigger $daily -Settings $updateSettings -User $env:USERNAME -RunLevel Limited -Force | Out-Null",
    "    try { Start-ScheduledTask -TaskName $StartTaskName -ErrorAction SilentlyContinue } catch {}",
    "    Register-StartProtocol",
    "    Write-UpdateLog ('Tareas: ' + $StartTaskName + ' (sesion + cada 2 min si se cayo) y ' + $TaskName + ' (03:20)')",
    "  } catch {",
    ...
    "}",
    "",
    "Write-UpdateLog ('Monitor SEO - ' + $Mode + ' para ' + $OrgName + ' ORG_ID ' + $OrgId)",
    "New-Item -ItemType Directory -Force -Path $InstallRoot | Out-Null",
    "New-Item -ItemType Directory -Force -Path $DataDir | Out-Null",
    "",
    "if ($Mode -eq 'Start') {",
