...
export function buildClientInstaller(input: InstallerInput): string {
  ...
  const lines = [
    ...
    "  try {",
    "    $psExe = Join-Path $env:SystemRoot 'System32\\WindowsPowerShell\\v1.0\\powershell.exe'",
    "    $cmd = '\"' + $psExe + '\" -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File \"' + $UpdaterFile + '\" -Mode Start'",
    "    $hkcu = [Microsoft.Win32.Registry]::CurrentUser",
    "    $key = $hkcu.CreateSubKey('Software\\Classes\\logicbus-seo')",
    "    $key.SetValue('', 'URL:Logicbus SEO')",
    "    $key.SetValue('URL Protocol', '')",
    "    $cmdKey = $hkcu.CreateSubKey('Software\\Classes\\logicbus-seo\\shell\\open\\command')",
    "    $cmdKey.SetValue('', $cmd)",
    "    $key.Close()",
    "    $cmdKey.Close()",
    "    Write-UpdateLog 'Ya esta listo en la web puede levantar el motor (logicbus-seo://start).'",
    "  } catch {",
