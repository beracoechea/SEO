...
function installerReadme(cmdName: string): string {
  return [
    ...
    "3. Si Windows pide administrador en un aviso aparte, pulsa Si. No abras el .cmd con Ejecutar como administrador.",
    "",
...
function installerCmdHeader(): string {
  return [
    ...
    "  exit /b 1",
    ")",
    "echo No abras este .cmd como administrador: Docker Desktop solo responde a tu usuario.",
...
export function buildClientInstaller(input: InstallerInput): string {
  ...
  const lines = [
    ...
    "param(",
    "  [ValidateSet('Install','Update','SetupAdmin')]",
    "  [string]$Mode = 'Install'",
    ")",
    ...
    "  $exe = Get-DockerExe",
    "  if (-not $exe) { throw 'No se encontro docker.exe. Abre Docker Desktop y reintenta.' }",
    "  & $exe @ArgList",
    "  if ($LASTEXITCODE -ne 0) { throw ('docker ' + ($ArgList -join ' ') + ' fallo (codigo ' + $LASTEXITCODE + ').') }",
    "}",
    "",
    ...
    "  Write-UpdateLog 'Abriendo Docker Desktop...'",
    "  Start-Process -FilePath $exe | Out-Null",
    "}",
    "",
    "function Invoke-ElevatedSetup {",
    "  $psExe = Join-Path $env:SystemRoot 'System32\\WindowsPowerShell\\v1.0\\powershell.exe'",
    "  Write-Host 'Se pedira administrador SOLO para Docker (si falta) y el firewall. Luego el motor corre con tu usuario.'",
    "  $p = Start-Process -FilePath $psExe -Verb RunAs -Wait -PassThru -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File', $ScriptPath, '-Mode', 'SetupAdmin')",
    "  if (-not $p) { throw 'Se cancelo el aviso de administrador.' }",
    "  if ($p.ExitCode -eq 3010) {",
    "    Write-Host 'Windows pide reiniciar para terminar Docker. Reinicia y vuelve a hacer doble clic en el .cmd (sin administrador).'",
    "    pause",
    "    exit 0",
    "  }",
    "  if ($p.ExitCode -ne 0) { throw ('La parte de administrador fallo (codigo ' + $p.ExitCode + ').') }",
    "}",
    "",
    ...
    "  if ($code -eq 3010) {",
    "    Write-Host 'Windows pide reiniciar para terminar Docker. Reinicia este PC y vuelve a hacer doble clic en el .cmd (sin administrador).'",
    ...
    "  try {",
    "    Invoke-Docker -ArgList @('compose','-f',$ComposeFile,'--env-file',$EnvFile,'stop')",
    "  } finally {",
    "    Pop-Location",
    "  }",
    "}",
    ...
    "  Write-Host 'La primera vez baja Chromium. Puede tardar varios minutos. No cierres esta ventana.'",
    "  Push-Location $InstallRoot",
    "  try {",
    "    Invoke-Docker -ArgList @('compose','-f',$ComposeFile,'--env-file',$EnvFile,'up','-d','--build')",
    "  } finally {",
    "    Pop-Location",
    "  }",
    "}",
    ...
    "  if (-not (Test-Path $ComposeFile)) { return }",
    "  Write-Host '--- docker compose logs (ultimas lineas) ---'",
    "  Push-Location $InstallRoot",
    "  try {",
    "    Invoke-Docker -ArgList @('compose','-f',$ComposeFile,'--env-file',$EnvFile,'logs','--tail','60')",
    "  } catch {}",
    "  Pop-Location",
    "}",
    "",
    ...
    "    Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger @($daily, $logon) -Settings $settings -User $env:USERNAME -RunLevel Limited -Force | Out-Null",
    ...
    "}",
    "",
    "if ($Mode -eq 'SetupAdmin') {",
    "  if (-not (Test-Admin)) { throw 'SetupAdmin requiere administrador.' }",
    "  Refresh-Path",
    "  if (-not (Get-DockerExe)) { Install-DockerDesktop }",
    "  try { Add-LocalGroupMember -Group 'docker-users' -Member $env:USERNAME -ErrorAction SilentlyContinue } catch {}",
    "  Open-LanFirewall",
    "  Write-UpdateLog 'Listo el paso de administrador (Docker + firewall).'",
    "  exit 0",
    "}",
    "",
    "if ((Test-Admin) -and $Mode -ne 'Update') {",
    "  Write-Host 'No ejecutes el .cmd como administrador: Docker Desktop no responde en esa ventana y el motor no arranca (ERR_CONNECTION_REFUSED en el navegador).'",
    "  Write-Host 'Cierra esto, deja Docker Desktop Running con tu usuario y haz doble clic en el .cmd.'",
    "  pause",
    "  exit 1",
    "}",
    "",
    ...
    "  Refresh-Path",
    "  Start-DockerDesktop",
    "  if (-not (Wait-Docker -Tries 36)) {",
    "    throw 'Docker Desktop no quedo Running. Abre Docker Desktop (ballena quieta en la bandeja), no uses \"Ejecutar como administrador\", y reintenta. Si acabas de instalar Docker, cierra sesion de Windows y vuelve a entrar.'",
    "  }",
    "}",
    "",
    "New-Item -ItemType Directory -Force -Path $InstallRoot | Out-Null",
    ...
    "if (-not $health) {",
    "  Show-ComposeLogs",
    "  throw 'El motor no respondio en http://127.0.0.1:8080/api/health. Deja Docker Desktop Running y reintenta el .cmd sin administrador.'",
    "}",
