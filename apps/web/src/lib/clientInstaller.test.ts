...
describe("clientInstaller", () => {
  ...
  it("incrusta el ORG_ID y no pide editar el env a mano", () => {
    ...
    expect(script).toContain("Install-PortablePython");
    expect(script).not.toContain("desktop.docker.com");
    expect(script).not.toContain("Install-DockerDesktop");
    expect(script).not.toContain("Wait-Docker");
  ...
  it("el archivo descargable es un .cmd que usa el PowerShell de Windows, no la Store", () => {
    ...
    expect(cmd).toContain("function Install-PortablePython");
    expect(cmd).toContain("function Start-Runtime");
    expect(cmd).not.toContain("Wait-Docker");
